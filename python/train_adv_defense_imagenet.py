...
parser.add_argument('--num-gpu', default=1, type=int, metavar='N', help='number of gpus to use (default: 1)')
parser.add_argument('-j', '--workers', default=1, type=int, metavar='N',
                    help='number of data loading workers (default: 4)')
parser.add_argument('--epochs', default=100, type=int, metavar='N', help='number of total epochs to run')
parser.add_argument('--decay-epochs', type=int, default=15, metavar='N', help='epoch interval to decay LR')
parser.add_argument('--start-epoch', default=0, type=int, metavar='N', help='manual epoch number (useful on restarts)')
parser.add_argument('-b', '--batch-size', default=16, type=int, metavar='N', help='mini-batch size (default: 256)')
parser.add_argument('--img-size', default=299, type=int, metavar='N', help='Input image dimension')
parser.add_argument('--opt', default='sgd', type=str, metavar='OPT', help='optimizer')
...
parser.add_argument('--mt', action='store_true', default=False, help='multi-task defense objective')
parser.add_argument('--co', action='store_true', default=False, help='optimize only defense classifier(s) parameters')
parser.add_argument('--df', action='store_true', default=False, help='dogfood attack with defense model')
...
def train(args, train_loader, model, criterion, optimizer, epoch):
    ...
    for i, batch in enumerate(train_loader):
        ...
        if isinstance(output, tuple):
            loss = multi_task.multi_loss(output, target_var, target_adv_var, is_adv_var, criterion)
            output = output[0]
        else:
        ...
        if i % 1000 == 0:
            save_checkpoint(
                {'epoch': epoch + 1, 'arch': 'FIXME', 'state_dict': model.state_dict(), 'best_prec1': top1.avg,
                    'optimizer': optimizer.state_dict(), }, False, filename='checkpoint-%d.pth.tar' % epoch)
...
def main():
    ...
    train_dataset = datasets.ImageFolder(train_dir, transforms.Compose(
        [transforms.RandomSizedCrop(args.img_size), transforms.RandomHorizontalFlip(), transforms.ToTensor()]))

    attack_cfgs = [{'attack_name': 'iterative', 'targeted': True, 'num_steps': 10, 'target_rand': True},
    ...
    attack_model_cfgs = [  # FIXME these are currently just test configs, need to setup properly
        {'models': ['inception_v3_tf']}, {'models': ['inception_resnet_v2', 'resnet34'], 'weights': [1.0, 1.0]},
        {'models': ['adv_inception_resnet_v2', 'inception_v3_tf']}, ]

    val_dataset = datasets.ImageFolder(val_dir, transforms.Compose(
        [transforms.Scale(int(math.floor(args.img_size / 0.875))), transforms.CenterCrop(args.img_size),
            transforms.ToTensor()]))
    ...
    if args.mt:
        if len(defense_ensemble) > 1:
            defense_model = multi_task.MultiTaskEnsemble(defense_model.models, use_features=False)
        else:
...
def train_adv_defense(args, defense_model, train_dataset, val_dataset, attack_model_cfgs, attack_cfgs):
    ...
    with torch.cuda.device(master_output_device):
        ...
        if args.opt == 'sgd':
            optimizer = torch.optim.SGD(get_opt_params(defense_model, classifier_only=args.co), args.lr,
                momentum=args.momentum, weight_decay=args.weight_decay)
        elif args.opt == 'adam':
            optimizer = torch.optim.Adam(get_opt_params(defense_model, classifier_only=args.co), args.lr,
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            save_checkpoint({'epoch': epoch + 1, 'arch': 'FIXME', 'state_dict': defense_model.state_dict(),
