...
def dpn68(num_classes=1000, pretrained=False, test_time_pool=True, extra=False):
    model = DPN(
    ...
    if pretrained:
        if extra:
            print('Warning: No "extra" weights available for dpn68')
        model.load_state_dict(model_zoo.load_url(model_urls['dpn68']))
    return model


def dpn68b(num_classes=1000, pretrained=False, test_time_pool=True, extra=True):
    model = DPN(
        small=True, num_init_features=10, k_r=128, groups=32,
        k_sec=(3, 4, 12, 3), inc_sec=(16, 32, 32, 64),
        num_classes=num_classes, test_time_pool=test_time_pool)
    if pretrained:
        if not extra:
            print('Warning: Only "extra" weights exist for this network')
...
def dpn98(num_classes=1000, pretrained=False, test_time_pool=True, extra=False):
    model = DPN(
    ...
    if pretrained:
        if extra:
            print('Warning: No "extra" weights available for dpn98')
        model.load_state_dict(model_zoo.load_url(model_urls['dpn98']))
    return model


def dpn131(num_classes=1000, pretrained=False, test_time_pool=True, extra=False):
    model = DPN(
    ...
    if pretrained:
        if extra:
            print('Warning: No "extra" weights available for dpn131')
        model.load_state_dict(model_zoo.load_url(model_urls['dpn131']))
    return model


def dpn107(num_classes=1000, pretrained=False, test_time_pool=True, extra=True):
    model = DPN(
    ...
    if pretrained:
        if not extra:
            print('Warning: Only "extra" weights available for dpn107')
