...
class SelectiveUniversal(object):
    ...
    def run(self):
        ...
        for img_idx, (input, _) in enumerate(loader):
            ...
            probs_var = self.target_ensemble(input_var)
            probs = probs_var.data.cpu().numpy()
            pred_class = autograd.Variable(torch.LongTensor(np.argsort(probs, axis=1)[:, -1])).cuda()
            ...
            for w_id, w_matrix in enumerate(self.w_matrices):
                ...
                probs_perturbed_var = self.target_ensemble(clamped)
                loss = -nllloss(torch.log(probs_perturbed_var + 1e-8), target=pred_class).data.cpu().numpy()
                if loss < best_loss:
                    best_loss = loss
                    best_w_id = w_id
